...
def plot_customizations(n_hidden_layers, neurons_in_layers, optimizer_choice, learning_rate, callbacks, patience, factor):
    customizations = f"\nCustomizations: \n# of Hidden Layers: {n_hidden_layers}"
    for i in range(n_hidden_layers):
        customizations += f"\n# of Neurons in H.L. {i+1}: {neurons_in_layers[i]}"
    customizations += f"\nOptimizer: {optimizer_choice} \nLearning Rate: {learning_rate} \nCallbacks: {callbacks} \nPatience: {patience} \nFactor: {factor}"
    plt.figtext(0.95, 0.05, customizations, wrap=True, horizontalalignment='right', fontsize=8)
...
plot_customizations(n_hidden_layers, neurons_in_layers, optimizer_choice, learning_rate, callbacks, patience, factor)
plt.show()
